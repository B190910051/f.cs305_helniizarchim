...
class Program {
    ...
    public void display()
    {
		...
		{
			Declaration d = decpart.get(i);			
			if(decs.length() > 1) decs += ", " + "<" + d.v + ", " + d.t + ">"; else decs += "<" + d.v + ", " + d.t + ">";
		}
		decs += "}";
    	System.out.println("    Declarations = " + decs);
    	System.out.println("  Block:");
    	System.out.println(this.BlockText(body, "  "));
    }
    
    private String BlockText (Block b, String tab) {
    	String val = "";
    ...
    private String BlockText (Statement s, String tab) {
        if (s instanceof Skip) return BlockText((Skip)s, tab + "  ");
        if (s instanceof Assignment)  return BlockText((Assignment)s, tab + "  ");
        if (s instanceof Conditional)  return BlockText((Conditional)s, tab + "  ");
        if (s instanceof Loop)  return BlockText((Loop)s, tab + "  ");
        if (s instanceof For)  return BlockText((For)s, tab + "  ");
        if (s instanceof Block)  return BlockText((Block)s, tab + "  ");
        throw new IllegalArgumentException("should never reach here");
    }
    
    private String BlockText (Expression e, String tab) {
        if (e instanceof Value) 
        { 
        	Value v = (Value)e;
        	if (v instanceof IntValue) return tab + "IntValue: " + v.intValue();        	       	        		
        	if (v instanceof BoolValue) return tab + "BoolValue: " + v.boolValue();
        	if (v instanceof CharValue) return tab + "CharValue: " + v.charValue();
        	if (v instanceof FloatValue) return tab + "FloatValue: " + v.floatValue();
        }
        if (e instanceof Variable) 
        {
            Variable v = (Variable)e;
            return tab + "Variable: " + v;
        }
        if (e instanceof Binary) {
            Binary b = (Binary)e;
            String val = tab + "Binary:\n";
            val += tab + "  Operator: " + b.op + "\n";
            val += this.BlockText(b.term1, tab + "  ") + "\n";
            val += this.BlockText(b.term2, tab + "  ");            
            return val;
        }
        if (e instanceof Unary) {
            Unary u = (Unary)e;
            String val = tab + "Unary:\n";
            val += tab + "  Operator: " + u.op + "\n";
            val += this.BlockText(u.term, tab + "  ");
            return val;
        }
        throw new IllegalArgumentException("should never reach here");
    }
  
    private String BlockText (Skip s, String tab) {
        return "";
    }
  
    private String BlockText (Assignment a, String tab) {
        String val = tab + "Assignment:\n";
        val += this.BlockText(a.target, tab + "  ") + "\n";
        val += this.BlockText(a.source, tab + "  ") + "\n";
    	return val;
    }   
  
    private String BlockText (Conditional c, String tab) {
    	String val = tab + "Conditional:\n";
        val += this.BlockText(c.test, tab + "  ") + "\n";
        val += tab + "  Thenbranch:\n";
        val += this.BlockText(c.thenbranch, tab + "  ");
        String v = this.BlockText(c.elsebranch, tab + "  ");
        if(v.length() > 0)
        {
        	val += tab + "  Elsebranch:\n";
            val += v + "\n";
    ...
    private String BlockText (Loop l, String tab) {
    	String val = tab + "Loop:\n";
        val += this.BlockText(l.test, tab + "  ") + "\n";
        val += tab + "  Block:\n";
        val += this.BlockText(l.body, tab);
    	return val;
    } 
    
    private String BlockText (For f, String tab) {
    	String val = tab + "For:\n";
    	val += this.BlockText(f.assign, tab + "  ") + "\n";
    	val += this.BlockText(f.test, tab + "  ") + "\n";
    	val += this.BlockText(f.a, tab + "  ") + "\n";
        val += tab + "  Block:\n";
        val += this.BlockText(f.body, tab);
    	return val;
    }  
}
...
class For extends Statement {
	// For = Assignment; Expression test; Assignment; Statement body
	Assignment assign;
	Assignment a;
	Expression test;
    Statement body;

    For( Assignment ass, Expression t, Assignment i , Statement b) {
    	assign = ass;  test = t;  a = i ; body = b;
